...
import { JobListingFetchError, JobListingSaveError } from '@/domain/job-listing/JobListingErrors';
import { JobListingFilter, JobListingRepository } from '@/domain/job-listing/JobListingRepository';
...
export class DrizzleJobListingRepository implements JobListingRepository {
  async find(filter: JobListingFilter): Promise<Result<JobListing | null, JobListingFetchError>> {
  ...
  async findMany(filter: JobListingFilter): Promise<Result<JobListing[], JobListingFetchError>> {
    return ResultAsync.fromPromise(
      db
        .select()
        .from(jobListingTable)
  ...
  async save(jobListing: JobListing): Promise<Result<void, JobListingSaveError>> {
    return ResultAsync.fromPromise(
  ...
  async count(filter: JobListingFilter): Promise<Result<number, JobListingFetchError>> {
    return ResultAsync.fromPromise(
      db
        .select({ count: count() })
        .from(jobListingTable)
  ...
  private buildFilterQuery(filter: JobListingFilter) {
    ...
    if (filter.createdAtBefore) {
      conditions.push(lt(jobListingTable.createdAt, filter.createdAtBefore.getTime()));
    }
    if (filter.createdAtAfter) {
      conditions.push(gt(jobListingTable.createdAt, filter.createdAtAfter.getTime()));
    }
